    ...
    fn apply(&self, expr: &Expr) -> Option<Transformation> {
        match expr {
            Expr::Integral {
                    ...
                    return Some(Transformation {
                        new_state: Expr::Add(
                            Box::new(Expr::Integral {
                                integrand: left.clone(),
                                variable: variable.clone(),
                            }),
                            Box::new(Expr::Integral {
                                integrand: right.clone(),
                                variable: variable.clone(),
                            }),
                        ),
                ...
                crate::calculus::simple_integrate(integrand, variable).map(|new_state| {
                    Transformation {
                        new_state,
                        precision_percent: 100.0,
...
fn simplify(expr: &Expr) -> Option<Expr> {
    match expr {
        Expr::Add(l, r) => {
            if let Some(s) = simplify(l) {
                return Some(Expr::Add(Box::new(s), r.clone()));
            }
            if let Some(s) = simplify(r) {
                return Some(Expr::Add(l.clone(), Box::new(s)));
            }
            match (&**l, &**r) {
        ...
        Expr::Mul(l, r) => {
            if let Some(s) = simplify(l) {
                return Some(Expr::Mul(Box::new(s), r.clone()));
            }
            if let Some(s) = simplify(r) {
                return Some(Expr::Mul(l.clone(), Box::new(s)));
            }
            match (&**l, &**r) {
                (Expr::Const(a), Expr::Const(b)) => Some(Expr::Const(a * b)),
        ...
        Expr::Sub(l, r) => {
            if let Some(s) = simplify(l) {
                return Some(Expr::Sub(Box::new(s), r.clone()));
            }
            if let Some(s) = simplify(r) {
                return Some(Expr::Sub(l.clone(), Box::new(s)));
            }
            match (&**l, &**r) {
                (Expr::Const(a), Expr::Const(b)) => Some(Expr::Const(a - b)),
                (_, Expr::Const(c)) if *c == 0.0 => Some(*l.clone()),
                (Expr::Const(c), _) if *c == 0.0 => {
                    Some(Expr::Mul(Box::new(Expr::Const(-1.0)), r.clone()))
                }
        ...
        Expr::Div(l, r) => {
            if let Some(s) = simplify(l) {
                return Some(Expr::Div(Box::new(s), r.clone()));
            }
            if let Some(s) = simplify(r) {
                return Some(Expr::Div(l.clone(), Box::new(s)));
            }
            match (&**l, &**r) {
                (Expr::Const(a), Expr::Const(b))
                    if *b != 0.0 && (a / b) == ((a / b) as i64) as f64 =>
                {
                    Some(Expr::Const(a / b))
                }
        ...
        Expr::Sin(inner) => {
            if let Some(s) = simplify(inner) {
                return Some(Expr::Sin(Box::new(s)));
            }
            if matches!(**inner, Expr::Const(c) if c == 0.0) {
        ...
        Expr::Cos(inner) => {
            if let Some(s) = simplify(inner) {
                return Some(Expr::Cos(Box::new(s)));
            }
            if matches!(**inner, Expr::Const(c) if c == 0.0) {
        ...
        Expr::Tan(inner) => {
            if let Some(s) = simplify(inner) {
                return Some(Expr::Tan(Box::new(s)));
            }
            if matches!(**inner, Expr::Const(c) if c == 0.0) {
                return Some(Expr::Const(0.0));
            }
        ...
        Expr::Pow(base, exp) => {
            if let Some(s) = simplify(base) {
                return Some(Expr::Pow(Box::new(s), exp.clone()));
            }
            if let Some(s) = simplify(exp) {
                return Some(Expr::Pow(base.clone(), Box::new(s)));
            }
            match &**exp {
...
impl Transform for AlpesIBP {
    fn apply(&self, expr: &Expr) -> Option<Transformation> {
        ...
        Some(Transformation {
            ...
            rule: RuleType::IntegrationByParts {
                u: *u.clone(),
                dv: *dv.clone(),
            },
        })
...
impl Transform for Substitution {
    fn apply(&self, expr: &Expr) -> Option<Transformation> {
        ...
        let _ = (
            crate::calculus::derive(left, variable),
            crate::calculus::derive(right, variable),
        );
        None
    }
}